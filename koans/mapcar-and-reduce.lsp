...
(define-test test-mapcar-multiple-lists
  ...
  (assert-equal (list (* 1 4) (* 2 5) (* 3 6)) (mapcar #'* '(1 2 3) '(4 5 6)))
...
(define-test test-transpose-using-mapcar
  ...
  (defun transpose (L) (apply #'mapcar (cons #'list L))) ; cool!
  (assert-equal '((1 4 7)
                  (2 5 8)
                  (3 6 9))
                (transpose '((1 2 3)
...
(define-test test-reduce-right-to-left
  ...
  (assert-equal (+ 4 3 2 1) (reduce #'+ '(1 2 3 4) :from-end t))
  (assert-equal (expt 2 (expt 3 2)) (reduce #'expt '(2 3 2) :from-end t))) ; the second one took me some time to understand, :from-end turns it into right-associative function => 2^(3^2) = 512
...
(define-test test-mapcar-and-reduce
  ...
  (assert-equal 310 (inner '(10 20 30) '(4 3 7)))) ; much cool, many expressive
